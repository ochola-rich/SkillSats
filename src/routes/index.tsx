  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] p-6 text-white">
      <header className="mb-10 max-w-7xl mx-auto">
        ...
        <p className="text-gray-400">Master new skills and pay creators directly with Bitcoin Lightning.</p>
      </header>
      ...
      <div className="max-w-7xl mx-auto">
        {isLoading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="bg-[#111118] border border-white/10 rounded-xl p-4 animate-pulse">
            ...
            {videos?.map((video) => (
              ...
              >
                <div className="bg-[#16161f] h-36 rounded-lg mb-4 flex items-center justify-center relative overflow-hidden">
                   <span className="material-symbols-outlined text-4xl text-white/20 group-hover:scale-110 transition-transform">play_circle</span>
                </div>
                <div className="flex justify-between items-start mb-1">
                  <h3 className="font-bold text-white text-lg line-clamp-1">{video.title}</h3>
                  {video.isFree ? (
                    <span className="bg-green-500/20 text-green-400 text-[10px] font-bold px-2 py-0.5 rounded uppercase">FREE</span>
                  ) : (
                    <span className="text-yellow-500 font-mono text-sm">⚡ {video.priceSats} sats</span>
                  )}
                </div>
