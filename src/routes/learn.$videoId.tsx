...
function LearnVideo() {
  ...
  const [purchaseData, setPurchaseData] = useState<{ payment_request: string; r_hash: string } | null>(null);
  ...
  if (!meta) return <div className="p-10 text-white bg-[#0a0a0f] min-h-screen">Video not found</div>;
            ...
            <div className="absolute inset-0 flex items-center justify-center p-8 text-center paywall-overlay">
              {!purchaseData ? (
                <div className="max-w-md">
                  <span className="material-symbols-outlined text-6xl text-yellow-500 mb-4">lock</span>
                  ...
                  >
                    {purchaseMutation.isPending ? "Generating Invoice..." : `Pay ⚡ ${meta.priceSats} Sats`}
                  </button>
                </div>
              ) : (
                <div className="flex flex-col items-center">
                  ...
                  <div className="text-center mb-6">
                    <p className="text-yellow-500 font-mono text-xl font-bold mb-2">⚡ {meta.priceSats} sats</p>
          ...
          <div className="bg-[#111118] border border-white/10 rounded-xl p-6 h-fit">
            ...
            <div className="space-y-4 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-gray-300">{new Date(meta.createdAt).toLocaleDateString()}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">Access</span>
                <span className="text-gray-300">{meta.isFree ? "Lifetime Free" : "One-time Purchase"}</span>
              </div>
            </div>
          </div>
        </div>
