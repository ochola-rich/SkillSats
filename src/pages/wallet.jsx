      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
          <div className="bg-gray-900 border border-gray-800 p-8 rounded-lg text-center">
            ...
            <h2 className="text-5xl font-bold text-yellow-400 mb-2">⚡ {user?.balanceSats?.toLocaleString() || 0}</h2>
          ...
          <div className="bg-gray-900 border border-gray-800 p-6 rounded-lg text-white">
            ...
            <form onSubmit={handleWithdraw} className="space-y-4">
              <div>
                <label className="block text-sm text-gray-400 mb-1">Lightning Invoice (payment_request)</label>
                <textarea
                  required
                  rows="3"
                  value={invoice}
        ...
        <div className="bg-gray-900 border border-gray-800 p-6 rounded-lg">
          ...
          {adHistory.length === 0 ? (
            ...
            <div className="space-y-3">
              {adHistory.map((watch) => (
                <div key={watch.id} className="flex items-center justify-between p-3 bg-gray-950 border border-gray-800 rounded">
                  <div>
                    <p className="text-white text-sm">Watched Ad</p>
                    <p className="text-xs text-gray-500">{new Date(watch.watchedAt).toLocaleDateString()}</p>
                  </div>
                  <span className="text-yellow-400 font-bold text-sm">+ {watch.earnedSats} sats</span>
                </div>
              ))}
            </div>
          )}
